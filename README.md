...
After each mission in Helldivers 2, the game displays a results screen with the player's statistics: kills, deaths, samples collected, objectives completed, etc. This Discord bot takes a screenshot of that screen and generates a narrative report written in the game's universe.

The report is written in one of 5 styles:

| Style | Description |
|-------|-------------|
| **Heroique** | Epic chronicle, martial tone, celebration of the Helldiver's exploits |
...
[Validation] -- format + size (max 10 MB)
    |
    v
[Pixtral] -- vision model, extracts stats as structured JSON
    |
    v
[Mistral Small] -- generates narrative text in the chosen style
    |
    v
[Discord Embed] -- visual formatting with stats + narrative
```

Two separate API calls are made to Mistral:
1. **Pixtral** (vision model, more expensive): reads the image and extracts structured data
2. **Mistral Small** (text model, cheap): writes the narrative report from the extracted stats
...
- `/report <image> [style] [lang]` — Analyze a screenshot and generate a report
- `/stats [lang]` — View player history (total kills, K/D, recent missions)
...
This creates:
- A Stardust instance (1 vCPU, 1 GB RAM)
- A 10 GB volume for the SQLite database
- A security group (all inbound traffic dropped except SSH)
- A cloud-init bootstrap that installs Node.js 20 and sets up the systemd service
...
- **Firewall**: Scaleway security group drops all inbound traffic on all ports, except SSH (port 22). The bot only makes outbound connections (WebSocket to Discord, HTTPS to Mistral) so no other port needs to be open
...
nano /opt/helldivers/.env
# Paste the variables (DISCORD_TOKEN, DISCORD_CLIENT_ID, MISTRAL_API_KEY, DATABASE_URL)
```
...
- **Deploy** (`deploy.yml`) — Runs on push to `main` only: builds the project, rsyncs to the server via SSH, installs production deps, and restarts the systemd service
...
│   ├── types.ts              # Shared types (MissionStats, Style, Language)
│   ├── commands/
...
4. Inside the queue: Pixtral extraction → Mistral narrative → upsert player in DB → save report → send embed
...
- Aggregated totals (missions, kills, deaths, samples) via SQL `sum()`
...
Sends the image to Pixtral with a prompt requesting strict JSON output:
...
Each style corresponds to a different **system prompt** that defines the narrator's personality. The **user prompt** contains the actual mission stats and a language instruction (`"Redige en francais."` or `"Write in English."`).
...
- **reports** — mission statistics, generated narrative, style used, linked to a player via foreign key
...
- Stats as inline fields (3 per row in Discord)
...
- `vm.tf`: Stardust instance + data volume + SSH key
- `network.tf`: security group (inbound SSH only, restricted to your IP)
- `cloud-init.yaml`: first-boot bootstrap (Node.js 20, `helldivers` user, systemd service, volume mount, SSH hardening, fail2ban, automatic security updates)
- `outputs.tf`: server public IP

### `deploy.sh`

One-command deployment script: local build, rsync to server, production dependency install, systemd service restart.
